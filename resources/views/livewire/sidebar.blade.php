    ...
    <div class="bg-white fixed h-full lg:w-64 w-20 shadow ease-in-out duration-700 overflow-y-auto">
        ...
        <div class="flex flex-col mt-3">
            ...
            <a class="nav-link{{ request()->routeIs('notification') ? "-active" : '' }}" href="{{ route('notification') }}">
                  ...
                  <span class="hidden lg:block">
                    Notification @if(auth()->user()->unreadNotifications->count() > 0)<div class="bg-green-700 px-3 py-2 rounded-full text-white inline">{{ auth()->user()->unreadNotifications->count() }}</div>@endif
                  </span>
            </a>
